...
myNameNode <- "wasb://dalton@aapocblob.blob.core.windows.net"
myPort <- 0
...
inputfile_bDat <- file.path(bigDataDirRoot,"bDat.csv")
bDat_text <- RxTextData(file = inputfile_bDat, missingValueString = "NA", fileSystem = hdfsFS)
bDat <- rxImport(bDat_text)

inputfile_bDatwoNAPOA <- file.path(bigDataDirRoot,"bDatwoNAPA.csv")
